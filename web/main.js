...
function drawStopLine() {
  ...
  if (stopPrice > 0) {
    stopLine = candleSeries.createPriceLine({
      price: stopPrice, color: "#ff9800", lineWidth: 1,
      lineStyle: LightweightCharts.LineStyle.Dashed,
      axisLabelVisible: true, title: "停損",
    });
  }
}
function clearStop() { stopPrice = 0; slInput.value = ""; drawStopLine(); }
slInput.onchange = () => {
  stopPrice = Number(slInput.value) || 0;
  drawStopLine();
  setStatus(stopPrice > 0 ? `✓ 已設定停損價 ${stopPrice}` : "已清除停損價");
};
